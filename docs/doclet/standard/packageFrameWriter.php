...
class PackageFrameWriter extends HTMLWriter
{
	...
	function &_allItems(&$rootDoc)
    {
		...
		usort($classes, function($a,$b) { return strcmp($a->packageName(), $b->packageName()) ? strcmp($a->packageName(), $b->packageName()) : strcmp($a->name(), $b->name()); });
		...
		if ($classes) {
			...
			foreach($classes as $name => $class) {
				...
				if ($pname != ($classes[$name]->packageName())) {
					$pname = $classes[$name]->packageName();
					echo '<li><h3><a href="'.$package->asPath().'/package-frame.html">'.$pname.'</a></h3></li>';
				}
